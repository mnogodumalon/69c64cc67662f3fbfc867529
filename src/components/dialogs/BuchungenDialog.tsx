...
export function BuchungenDialog({ open, onClose, onSubmit, defaultValues, recordId, mitgliederList, kurseList, enablePhotoScan = true, enablePhotoLocation = true }: BuchungenDialogProps) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "mitglied": string | null, // Display name from Mitglieder (see <available-records>)\n  "kurs": string | null, // Display name from Kurse (see <available-records>)\n  "zahlungsstatus": LookupValue | null, // Zahlungsstatus (select one key: "ausstehend" | "bezahlt" | "storniert") mapping: ausstehend=Ausstehend, bezahlt=Bezahlt, storniert=Storniert\n  "zahlungsart": LookupValue | null, // Zahlungsart (select one key: "bar" | "ueberweisung" | "ec_karte" | "kreditkarte" | "paypal") mapping: bar=Bar, ueberweisung=Überweisung, ec_karte=EC-Karte, kreditkarte=Kreditkarte, paypal=PayPal\n  "betrag": number | null, // Betrag (€)\n  "anmerkungen": string | null, // Anmerkungen\n  "buchungsdatum": string | null, // YYYY-MM-DD\n}`;
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    'mitglied': (
      <div key="mitglied" className="space-y-1.5">
        ...
        <Combobox
          id="mitglied"
          placeholder="Welches Mitglied bucht?"
      ...
      <div key="zahlungsart" className="space-y-1.5">
        ...
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.zahlungsart) === 'ec_karte'}
            onClick={() => setFields(f => ({ ...f, zahlungsart: (lookupKey(f.zahlungsart) === 'ec_karte' ? undefined : 'ec_karte') as any }))}
            className={`inline-flex items-center rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.zahlungsart) === 'ec_karte'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
    ...
    'anmerkungen': (
      <div key="anmerkungen" className="space-y-1.5">
        ...
        <Textarea
          id="anmerkungen"
          placeholder="Besonderheiten, Rabattgründe, Gutschein-Info"
    ...
    'buchungsdatum': (
      <div key="buchungsdatum" className="space-y-1.5">
        ...
        <DatePicker
          id="buchungsdatum"
          placeholder="Wann ist die Buchung erfolgt?"
          mode="date"
  ...
  const FIELD_LABELS: Record<string, string> = {"mitglied": "Mitglied", "kurs": "Kurs", "zahlungsstatus": "Zahlungsstatus", "zahlungsart": "Zahlungsart", "betrag": "Betrag (€)", "anmerkungen": "Anmerkungen", "buchungsdatum": "Buchungsdatum"};
  const CURRENCY_KEYS = new Set<string>(["betrag"]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"mitglied": {"vorname": "Vorname", "nachname": "Nachname", "geburtsdatum": "Geburtsdatum", "geschlecht": "Geschlecht", "email": "E-Mail-Adresse", "telefon": "Telefonnummer", "strasse": "Straße", "hausnummer": "Hausnummer", "postleitzahl": "Postleitzahl", "stadt": "Stadt", "mitgliedschaft_typ": "Mitgliedschaftstyp", "mitglied_seit": "Mitglied seit", "notfall_vorname": "Notfallkontakt Vorname", "notfall_nachname": "Notfallkontakt Nachname", "notfall_telefon": "Notfallkontakt Telefon", "gesundheitshinweise": "Gesundheitliche Hinweise / Einschränkungen"}, "kurs": {"kursname": "Kursname", "yoga_stil": "Yoga-Stil", "beschreibung": "Kursbeschreibung", "niveau": "Niveau", "wochentag": "Wochentag(e)", "uhrzeit": "Uhrzeit", "dauer_minuten": "Dauer (Minuten)", "startdatum": "Startdatum", "enddatum": "Enddatum", "raum": "Raum / Ort", "max_teilnehmer": "Maximale Teilnehmerzahl", "kursgebuehr": "Kursgebühr (€)", "trainer": "Trainer"}};
