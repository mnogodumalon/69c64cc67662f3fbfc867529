...
export function TrainerDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: TrainerDialogProps) {
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults((defaultValues ?? {}) as Record<string, unknown>, formEnhancements.defaults) as Partial<Trainer['fields']>);
      setPreview(null);
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'spezialisierungen': (
      <div key="spezialisierungen" className="space-y-1.5">
    ...
    'ausbildung': (
      <div key="ausbildung" className="space-y-1.5">
        <Label htmlFor="ausbildung">Ausbildung & Zertifikate</Label>
        <Textarea
          id="ausbildung"
          placeholder="Zertifikate, Ausbildungsstätten, Abschlüsse"
    ...
    'erfahrung_jahre': (
      <div key="erfahrung_jahre" className="space-y-1.5">
        <Label htmlFor="erfahrung_jahre">Berufserfahrung (Jahre)</Label>
        <Input
          id="erfahrung_jahre"
          type="number"
          step="any"
          {...numberInputProps(formEnhancements, 'erfahrung_jahre')}
          placeholder="z. B. 8"
          value={fields.erfahrung_jahre !== undefined ? fields.erfahrung_jahre : (computedValues['erfahrung_jahre'] ?? '')}
          onChange={e => setFields(f => ({ ...f, erfahrung_jahre: clampNumberValue(formEnhancements, 'erfahrung_jahre', e.target.value) }))}
        />
      </div>
    ),
    'biografie': (
      <div key="biografie" className="space-y-1.5">
        <Label htmlFor="biografie">Biografie</Label>
        <Textarea
          id="biografie"
          placeholder="Persönlicher Hintergrund, Philosophie, Leidenschaft"
  ...
  const FIELD_LABELS: Record<string, string> = {"nachname": "Nachname", "email": "E-Mail-Adresse", "telefon": "Telefonnummer", "spezialisierungen": "Yoga-Spezialisierungen", "ausbildung": "Ausbildung & Zertifikate", "erfahrung_jahre": "Berufserfahrung (Jahre)", "biografie": "Biografie", "foto": "Foto", "vorname": "Vorname"};
