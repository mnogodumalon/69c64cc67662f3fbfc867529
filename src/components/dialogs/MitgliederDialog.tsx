...
export function MitgliederDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: MitgliederDialogProps) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "vorname": string | null, // Vorname\n  "nachname": string | null, // Nachname\n  "geburtsdatum": string | null, // YYYY-MM-DD\n  "geschlecht": LookupValue | null, // Geschlecht (select one key: "weiblich" | "maennlich" | "divers" | "keine_angabe") mapping: weiblich=Weiblich, maennlich=Männlich, divers=Divers, keine_angabe=Keine Angabe\n  "email": string | null, // E-Mail-Adresse\n  "telefon": string | null, // Telefonnummer\n  "strasse": string | null, // Straße\n  "hausnummer": string | null, // Hausnummer\n  "postleitzahl": string | null, // Postleitzahl\n  "stadt": string | null, // Stadt\n  "mitgliedschaft_typ": LookupValue | null, // Mitgliedschaftstyp (select one key: "einzelstunde" | "zehnerkarte" | "monat" | "jahr" | "schnupperkurs") mapping: einzelstunde=Einzelstunde, zehnerkarte=10er-Karte, monat=Monatsmitgliedschaft, jahr=Jahresmitgliedschaft, schnupperkurs=Schnupperkurs\n  "mitglied_seit": string | null, // YYYY-MM-DD\n  "notfall_vorname": string | null, // Notfallkontakt Vorname\n  "notfall_nachname": string | null, // Notfallkontakt Nachname\n  "notfall_telefon": string | null, // Notfallkontakt Telefon\n  "gesundheitshinweise": string | null, // Gesundheitliche Hinweise / Einschränkungen\n}`;
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
      ...
      <div key="geschlecht" className="space-y-1.5">
        ...
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.geschlecht) === 'keine_angabe'}
            onClick={() => setFields(f => ({ ...f, geschlecht: (lookupKey(f.geschlecht) === 'keine_angabe' ? undefined : 'keine_angabe') as any }))}
            className={`inline-flex items-center rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.geschlecht) === 'keine_angabe'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
    ...
    'mitgliedschaft_typ': (
      <div key="mitgliedschaft_typ" className="space-y-1.5">
        <Label htmlFor="mitgliedschaft_typ">Mitgliedschaftstyp</Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.mitgliedschaft_typ) === 'einzelstunde'}
            onClick={() => setFields(f => ({ ...f, mitgliedschaft_typ: (lookupKey(f.mitgliedschaft_typ) === 'einzelstunde' ? undefined : 'einzelstunde') as any }))}
            className={`inline-flex items-center rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.mitgliedschaft_typ) === 'einzelstunde'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.mitgliedschaft_typ) === 'zehnerkarte'}
            onClick={() => setFields(f => ({ ...f, mitgliedschaft_typ: (lookupKey(f.mitgliedschaft_typ) === 'zehnerkarte' ? undefined : 'zehnerkarte') as any }))}
            className={`inline-flex items-center rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.mitgliedschaft_typ) === 'zehnerkarte'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.mitgliedschaft_typ) === 'monat'}
            onClick={() => setFields(f => ({ ...f, mitgliedschaft_typ: (lookupKey(f.mitgliedschaft_typ) === 'monat' ? undefined : 'monat') as any }))}
            className={`inline-flex items-center rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.mitgliedschaft_typ) === 'monat'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.mitgliedschaft_typ) === 'jahr'}
            onClick={() => setFields(f => ({ ...f, mitgliedschaft_typ: (lookupKey(f.mitgliedschaft_typ) === 'jahr' ? undefined : 'jahr') as any }))}
            className={`inline-flex items-center rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.mitgliedschaft_typ) === 'jahr'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.mitgliedschaft_typ) === 'schnupperkurs'}
            onClick={() => setFields(f => ({ ...f, mitgliedschaft_typ: (lookupKey(f.mitgliedschaft_typ) === 'schnupperkurs' ? undefined : 'schnupperkurs') as any }))}
            className={`inline-flex items-center rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.mitgliedschaft_typ) === 'schnupperkurs'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
    ...
    'mitglied_seit': (
      <div key="mitglied_seit" className="space-y-1.5">
        <Label htmlFor="mitglied_seit">Mitglied seit</Label>
        <DatePicker
          id="mitglied_seit"
          placeholder="Wann ist das Mitglied beigetreten?"
          mode="date"
          value={fields.mitglied_seit ?? null}
          onChange={v => setFields(f => ({ ...f, mitglied_seit: v ?? undefined }))}
        />
      </div>
    ),
    'notfall_vorname': (
      <div key="notfall_vorname" className="space-y-1.5">
    ...
    'notfall_nachname': (
      <div key="notfall_nachname" className="space-y-1.5">
    ...
    'gesundheitshinweise': (
      <div key="gesundheitshinweise" className="space-y-1.5">
        <Label htmlFor="gesundheitshinweise">Gesundheitliche Hinweise / Einschränkungen</Label>
        <Textarea
          id="gesundheitshinweise"
          placeholder="Allergien, Verletzungen, Einschränkungen oder Erkrankungen"
  ...
  const FIELD_LABELS: Record<string, string> = {"vorname": "Vorname", "nachname": "Nachname", "geburtsdatum": "Geburtsdatum", "geschlecht": "Geschlecht", "email": "E-Mail-Adresse", "telefon": "Telefonnummer", "strasse": "Straße", "hausnummer": "Hausnummer", "postleitzahl": "Postleitzahl", "stadt": "Stadt", "mitgliedschaft_typ": "Mitgliedschaftstyp", "mitglied_seit": "Mitglied seit", "notfall_vorname": "Notfallkontakt Vorname", "notfall_nachname": "Notfallkontakt Nachname", "notfall_telefon": "Notfallkontakt Telefon", "gesundheitshinweise": "Gesundheitliche Hinweise / Einschränkungen"};
