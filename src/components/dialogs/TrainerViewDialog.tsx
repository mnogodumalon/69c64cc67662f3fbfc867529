      ...
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-1">
            ...
            <p className="text-sm">{Array.isArray(record.fields.spezialisierungen) ? record.fields.spezialisierungen.map((v: any) => v?.label ?? v).join(', ') : '—'}</p>
          </div>
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">Ausbildung & Zertifikate</Label>
            <p className="text-sm whitespace-pre-wrap">{record.fields.ausbildung ?? '—'}</p>
          </div>
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">Berufserfahrung (Jahre)</Label>
            <p className="text-sm">{record.fields.erfahrung_jahre ?? '—'}</p>
          </div>
          <div className="space-y-1">
